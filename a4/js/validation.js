...
if (username.length>12|| username===""){
     usernameInvalidDiv.innerHTML = "<p>Invalid username, Maximum 12 characters Allowed</p>";
     isValid=false;
}
...
//Validate password
    ...
    if (!hasUpperCase.test(password)&& !hasLowerCase.test(password)&& !hasNumber.test(password)&& !hasSpecialChar.test(password)){
     passwordInvalidDiv.innerHTML = "<p>Password is invalid, Please include one of each: Upper Case, Lower Case, Number, and Special Character.</p>";
     isValid=false;
    }
...
//Set error if the form is invalidated in any way
if (!isValid) {
